    ...
    PausableUpgradeable
{
    ...
    event LogSetPositions(
        address[] holders,
        Position.InitParams[] _positions,
    ...
    event LogClaimRevenueDistribution(
        address indexed caller,
        uint256 indexed tokenId,
        uint256 value
    );
    ...
    modifier onlyBondsRegistry() {
        require(
            msg.sender == address(bondsRegistry),
            "only bonds registry allowed"
        );
        _;
    }
    ...
    modifier updateRevDis(uint256 _tokenId) {
        ...
        position.pendingRevDis = position
            .earnedRevDis(revDisPerToken)
            .toUint128();
    ...
    /// @inheritdoc IVesting
    function vestedUnderlyingFor(
        uint256 _tokenId
    ) external view virtual returns (uint256 vestedUnderlying) {
        vestedUnderlying = positions[_tokenId].vestedUnderlying();
    }

    /// @inheritdoc IVesting
    function availableUnderlyingFor(
        uint256 _tokenId
    ) public view virtual returns (uint256 availableUnderlying) {
        uint256 vestedUnderlying = positions[_tokenId].vestedUnderlying();
        uint256 positionAvailableBalance = positions[_tokenId].balance;
        availableUnderlying = vestedUnderlying > positionAvailableBalance
            ? positionAvailableBalance
            : vestedUnderlying;
    }

    /// @inheritdoc IVesting
    function pendingRevDisFor(
        uint256 _tokenId
    ...
    /// @inheritdoc IVesting
    function setBondsContract(
        IBondsRegistry _bondsRegistry
    ) external virtual onlyOwner {
        require(
            address(_bondsRegistry) != address(0),
            "invalid _bondsRegistry"
        );
    ...
    function setPositions(
        ...
        Position.InitParams[] calldata _positions
    ) external virtual override onlyOwner {
        require(_holders.length == _positions.length, "array length mismatch");
        uint256 totalUnderlying;
        for (uint256 i = 0; i < _holders.length; i++) {
            require(
                _positions[i].end > _positions[i].start &&
                    _positions[i].start > 0 &&
                    _positions[i].balance > 0,
    ...
    /// @inheritdoc IVesting
    function claimRevenueDistribution(
        uint256 _tokenId
    )
        external
        virtual
    ...
    function afterUnderlyingOffer(
        ...
        whenNotPaused
        onlyBondsRegistry
        onlyApprovedOrOwner(_caller, _tokenId)
        updateRevDis(_tokenId)
    {
    ...
    /// @inheritdoc IVesting
    function receiveVaultRewards(
        uint256 _reward
    ) external virtual whenNotPaused onlyVault {
        require(
            underlyingSupplied > 0 && _reward > 0,
            "invalid state or input"
        );

        revDisPerToken +=
            (_reward * Constants.BASE_MULTIPLIER) /
            underlyingSupplied;
    ...
    function _beforeTokenTransfer(
        ...
        require(
            msg.sender == owner() || isTransferAllowed,
            "ERC721 transfers not allowed"
        );
    }
